...
export const catsLoaded = createAction(
    "[Cats Page] Cats loaded successfuly",
    props<{ cats: Cat[] }>(),
);
export const errorCats = createAction("[Cats Page] Error loading cats");

export const loadBreeds = createAction(
    "[Cats Page] Search breed in API",
    props<{ term: string }>(),
);
export const breedsLoaded = createAction(
    "[Cats Page] Load breed from API",
    props<{ breeds: Breed[] }>(),
);

export const loadCatWithBreed = createAction(
    "[Cats Page] Search cat with passed breed in API",
    props<{ breed: Breed }>(),
);
...
const CatReducer = createReducer(
    ...
    on(errorCats, state => ({
        ...state,
        error: "Error while getting cats, retry later",
    })),
);
...
export class CatEffects {
    ...
    loadBreeds$ = createEffect(() =>
        this.actions$.pipe(
            ofType(loadBreeds),
            mergeMap(action =>
                this.catService.searchBreeds(action.term).pipe(
    ...
    loadCatWithBreed$ = createEffect(() =>
        this.actions$.pipe(
            ofType(loadCatWithBreed),
            mergeMap(action =>
                this.catService.searchCatWithBreed(action.breed).pipe(
                    map((cats: Cat[]) => catsLoaded({ cats })),
                    catchError(() => of(errorCats())),
                ),
            ),
